...
void udp_service::arp_answer_from_request(uint8_t * buf, uint8_t len)
{
    ...
    const ether_addr_t& src_mac = _iface.get_mac_addr();
    ...
    const ip_addr_t& src_ip = _iface.get_ip_addr();
...
void udp_service::reply_from_request(uint8_t * buf, char * data, uint8_t datalen, uint16_t port)
{
    ...
    uint16_t ck = udp_service::checksum(&buf[IP_SRC_P], 16 + datalen, 1);
 ...
 */
uint16_t udp_service::checksum(uint8_t * buf, uint16_t len, uint8_t type)
{
    uint32_t sum = 0;
...
void udp_service::send_to(ether_addr_t * dst_mac, ip_addr_t * dst_ip, uint16_t dst_port, uint8_t * data, uint8_t data_len)
{
	printf("udp ");
	_delay_ms(500);
    ...
    ck = udp_service::checksum(&buf[IP_SRC_P], 16 + data_len, 1);
    buf[UDP_CHECKSUM_H_P] = ck >> 8;
    buf[UDP_CHECKSUM_L_P] = ck & 0xff;
